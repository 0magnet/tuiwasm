...
[TinyGo build](https://0magnet.github.io/tuiwasm/tinygo/) — smaller, and
currently broken; see Building and serving.
...
`pterm` is not listed: it needs `-tags pterm`, see `shims/`. Nor is `widgets`,
which is tview and has not followed tcell to v3 — it is on the v2 branch.
...
| [`life`](https://0magnet.github.io/tuiwasm/?demo=life) | screen | Conway's life, coloured by how long a cell has lived |
...
The links go to the published site, which is built from the v2 branch. The
demos there are these demos — what differs is the tcell underneath them, which
is not something a demo can see.
...
The standard Go build is at the root because it is the one that works. The
TinyGo build, which would otherwise be preferred at a third of the size, is a
click away and currently broken: its linear memory is 3.3GB at load — declared
before any Go runs, not grown — and past 2GB TinyGo hands JavaScript negative
addresses, so every DataView write throws and the terminals stay blank. The
same source built with Go declares 29MB. See the flags in `build.sh`.
...
Markdown                         3             72              0            229
HTML                             2              0              4             94
Bourne Shell                     1              5             30             21
-------------------------------------------------------------------------------
TOTAL                           40            517            719           3098
-------------------------------------------------------------------------------
```

## Embedding the demos elsewhere

The demos do not depend on desk, or on this repository's own pages. Anything
that can put an xterm-go terminal somewhere can run them:
...
import (
    ...
    _ "github.com/0magnet/tuiwasm/demos/anim"   // importing registers them
    _ "github.com/0magnet/tuiwasm/demos/charts"
)

for _, d := range demos.All() { /* name, description, which shape */ }

d, _ := demos.Lookup("donut")
session, err := play.In(d, myTerminal, myElement)  // a terminal you own
defer session.Close()
```

`play.In` works out whether the demo writes text or paints cells, bridges a
tcell screen to the terminal when one is needed, sizes it from the terminal
rather than tcell's hardcoded 80x24, and runs it on its own goroutine because
`PollEvent` blocks and a page has one thread. `play.Mount` is the same thing
for a caller with no terminal of its own.

**Use `play.NewTerminal`, or call `play.FastRenderer` on your own terminal.**
Half of these are animations that repaint every cell every frame, and xterm-go's
DOM renderer builds a span per cell — on that path a page stops answering
rather than merely running slowly. WebGL draws the same cells from a glyph
atlas.

`demos`, `play`, `xtcell` and `xwrite` pull in no desk dependency, so embedding
the demos does not mean embedding a window manager.
