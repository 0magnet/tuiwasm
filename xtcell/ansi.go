...
func sgr(fg, bg int, attrs tcell.AttrMask, us tcell.UnderlineStyle) string {
	...
	// Styled underlines are the 4:n sub-parameter form. v3 carries underline only
	// as a style and no longer as an attribute bit, so this is the whole of it. Only plain 4 is sent,
	...
	if us != tcell.UnderlineStyleNone {
		p = append(p, "4")
	}
