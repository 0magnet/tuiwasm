...
// SetCell is the pre-Style way of setting a cell, kept because tcell's
// interface has it. The style comes first in the variadic runes.
...
func (s *Screen) cell(x, y int, str string, style tcell.Style) {
	...
	const us = tcell.UnderlineStyleNone
...
// post delivers an event, dropping it if nothing is reading.
//
...
func (s *Screen) HasPendingEvent() bool { return len(s.evch) > 0 }

// ChannelEvents forwards events until quit is closed or the screen is
// finalised, and closes ch on the way out.
//
// Closing ch is the contract callers rely on to notice a screen going away:
// an animation loop selects on it and returns when it closes, which is how a
// closed window ends its goroutine rather than leaving it spinning.
...
func (s *Screen) CanDisplay(r rune, checkFallbacks bool) bool {
	...
	// The terminal is UTF-8 and the font is the page's business, so there is
	// nothing here that can say no.
	return true
}
...
// HasKey reports that the browser can deliver a key. It can deliver all of
// them: keys come from a DOM keydown event, not from a terminfo table.
...
func (s *Screen) HasMouse() bool                  { return false }
...
// Accepted and ignored: tcell's contract for a terminal that declines a
// clipboard read is that no event arrives, so a caller already copes.
