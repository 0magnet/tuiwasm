...
// Closing the queue is the contract a caller relies on to notice a screen going
// away: an animation loop reads it and returns when it closes, which is how a
// closed window ends its goroutine instead of leaving it spinning.
//
// v3 hands the channel out directly, so there is no forwarding goroutine to
// close a second channel — Fini closes this one.
func TestTheEventQueueClosesOnFini(t *testing.T) {
	s, _ := newTestScreen(t, 20, 5)
	s.Fini()

	for range s.EventQ() { //nolint:revive // draining until closed is the point
	}
}
...
func TestPlainRunesArriveAsRunes(t *testing.T) {
	// v3 carries the text of a key rather than a rune, for grapheme clusters.
	k, str, mod, ok := decodeKey("q", false, false, false, false)
	if !ok || k != tcell.KeyRune || str != "q" || mod != tcell.ModNone {
		t.Errorf("q decoded to key %v text %q mod %v", k, str, mod)
	}
}
